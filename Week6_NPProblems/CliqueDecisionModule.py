...
def independent_set_decision(H, s):
    G = {}
    for node in H.keys():
        for node_1 in H.keys():
            if node != node_1 and node_1 not in H[node]:
                make_link(G, node, node_1)
        if node not in G:
            G[node] = {}
...
class test_independant_set(unittest.TestCase):
    def test(self):
        self.assertFalse(independent_set_decision({1:{2:1}, 2:{1:1}}, 2))
        self.assertTrue(independent_set_decision({1:{2:1, 3:1}, 2:{1:1}, 3:{1:1}, 4:{}}, 3))
        self.assertFalse(independent_set_decision({1:{2:1, 3:1}, 2:{1:1}, 3:{1:1}, 4:{}}, 4))
        self.assertTrue(independent_set_decision({1:{}}, 1))
        self.assertTrue(independent_set_decision({1:{2:1}, 2:{1:1}}, 1))

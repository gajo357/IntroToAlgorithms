...
class test_heap(unittest.TestCase):
    def _test_location(self, heap, location):
        for n, i in location.items():
            self.assertEqual(heap[i], n)
    ...
    def test_heap(self):
        ...
        self._test_location(h, location)
        old_min = (-float('inf'), None)
        while len(h) > 0:
            new_min = heappopmin(h, location)
            self._test_location(h, location)
            self.assertTrue(val_(old_min) <= val_(new_min))
            old_min = new_min    
    ...
    def test_add_and_modify(self):
        ...
        self.assertEqual((-1, 'k'), heappopmin(h, location))
        decrease_val(h, location, (6, 'c'), (-1, 'c'))
        self.assertEqual((-1, 'c'), heappopmin(h, location))
        self._test_location(h, location)
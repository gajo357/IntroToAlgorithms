...
def get_time(timestring):
    (hourstring, minutesstring) = timestring.split(':')
    return int(hourstring) * 60 + int(minutesstring)

class flight_info:
    def __init__(self, FlightNumber, DepartureTime, ArrivalTime, Cost):
        self.flightNumber = FlightNumber
        self.departureTime = DepartureTime
        self.arrivalTime = ArrivalTime   
        self.cost = Cost
    def __add__(self, another):
        # extend the flight number, departure is still the same, arrival is the arrival of the second flight, cost is the sum
        flights = []
        deptime = self.departureTime
        if len(self.flightNumber) > 0:
            flights.extend(self.flightNumber)
        else:
            deptime = another.departureTime
        flights.extend(another.flightNumber)
        return flight_info(flights, deptime, another.arrivalTime, self.cost + another.cost)
    ...
    def laterThen(self, another):
        return self.arrivalTime > another.departureTime
...
def build_graph(flights):
    G = {}
    for FlightNumber, Origin, Destination, DepartureTime, ArrivalTime, Cost in flights:
        make_link(G, Origin, FlightNumber, flight_info([FlightNumber], get_time(DepartureTime), 0, Cost))
        make_link(G, FlightNumber, Destination, flight_info([], 0, get_time(ArrivalTime), 0))
    return G

# extended Dijkstra, keeps the path to each node
def dijkstra_shortest_path_flights(G, start_node, end_node):
    # distances so far
    dist_so_far = []
    ...
    node = heap_node(start_node, flight_info([], 0, 0, 0))

    # add it to the heap and to the dictionary
    dist_so_far.append(node)
    
    while len(dist_so_far) > 0:

        # find the closes node
        wNode = dist_so_far.pop()
        ...
        for x in G[w]:
            ...
            #if x in final_dist:
            #    continue
            # is it feasible to reach this flight
            if final_dist[w].laterThen(G[w][x]):
                continue

            # calculate the new distance
            new_value = (final_dist[w] + G[w][x])

            # we have found one possible path
            if x == end_node:
                possible_paths.append(new_value)
            else:
    ...
    for p in possible_paths[1:]:
        if p < path:
            path = p
        elif p == path and p.total_flight_time() < path.total_flight_time():
            path = p
    return path

def find_best_flights(flights, origin, destination):
    G = build_graph(flights)
    cost = dijkstra_shortest_path_flights(G, origin, destination)
    if(cost is None):
        return None
    # your code here
    return cost.flightNumber

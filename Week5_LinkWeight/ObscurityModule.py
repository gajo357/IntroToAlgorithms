...
def make_link(G, node1, node2, weight):
    ...
    new_weight = weight
    if(node2 in G[node1]):
        new_weight = min((G[node1])[node2], weight)
    
    (G[node1])[node2] = new_weight
        
    if node2 not in G:
        G[node2] = {}
    (G[node2])[node1] = new_weight
    return G
...
def read_file(filename):
    ...
    with open(filename) as f:
        tsv = unicode_csv_reader(f, delimiter='\t')
        for (actor, title, year) in tsv:
            full_name = make_full_title(title, year)

            make_link(G, actor, full_name, 0)
...
def read_scores_file(filename):
    ...
    with open(filename) as f:
        tsv = unicode_csv_reader(f, delimiter='\t')
        for (title, year, weight) in tsv:
            full_name = make_full_title(title, year)
            weights[full_name] = float(weight)
    return weights

def make_full_title(title, year):
    return title + ' ' + year

def create_actors_graph(full_graph, actors, weights):
    actors_graph = {}
    for actor in actors:
        for movie in full_graph[actor]:
            for co_star in full_graph[movie]:
                if co_star < actor:
                    weight = weights.get(movie, 0)
                    make_link(actors_graph, actor, co_star, weight)
    return actors_graph

class test_obscurity(unittest.TestCase):
    def test(self):
        weights = read_scores_file('imdb-weights.tsv') 
        (actors, full_graph) = read_file('imdb-1.tsv')
        actors_graph = create_actors_graph(full_graph, actors, weights)
